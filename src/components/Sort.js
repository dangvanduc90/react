            ...
            <div className="dropdown">
                ...
                <ul className="dropdown-menu" aria-labelledby="dropdownMenu1">
                    ...
                    <li onClick={() => this.onClick('status', 1)}>
                        <a role="button" className={(sort.by === 'status' && sort.value === 1) ? "sort_selected" : ""}>Trạng Thái Kích Hoạt</a>
                    </li>
                    <li onClick={() => this.onClick('status', -1)}>
                        <a role="button" className={(sort.by === 'status' && sort.value === -1) ? "sort_selected" : ""}>Trạng Thái Ẩn</a>
                    </li>
                </ul>
            </div>
        )
...
const mapDispatchToProps = (dispatch, props) => {
    return {
        onSort: (sort) => {
            dispatch(actions.sort_task(sort));
        }
    }
};
